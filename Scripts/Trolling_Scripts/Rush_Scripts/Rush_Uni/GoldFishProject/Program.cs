...
namespace WinUpdate
{
    class Program : IDisposable
    {
        ...
        {
            using (Stream left = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("FishTank.Resources.{0}.Unicorn_Left.png", color)))
            using (Stream right = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("FishTank.Resources.{0}.Unicorn_Right.png", color)))
            using (Bitmap leftBitmap = new Bitmap(left))
            using (Bitmap rightBitmap = new Bitmap(right))
                return new FramesetPair(new Frameset(leftBitmap, 3), new Frameset(rightBitmap, 3));
        }
