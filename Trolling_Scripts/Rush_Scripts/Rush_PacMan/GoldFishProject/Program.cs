...
namespace WinUpdate
{
    class Program : IDisposable
    {
        ...
        {
            m_framesets.Add(LoadFramesets("Cat"));
        }

        private static FramesetPair LoadFramesets(string color)
        {
            using (Stream left = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("FishTank.Resources.{0}.Orange_Left.png", color)))
            using (Stream right = Assembly.GetExecutingAssembly().GetManifestResourceStream(string.Format("FishTank.Resources.{0}.Orange_Right.png", color)))
            using (Bitmap leftBitmap = new Bitmap(left))
            using (Bitmap rightBitmap = new Bitmap(right))
                return new FramesetPair(new Frameset(leftBitmap, 2), new Frameset(rightBitmap, 2));
        }
