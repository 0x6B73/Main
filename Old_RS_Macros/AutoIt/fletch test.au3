...
$Main_Gui = GUICreate("", 400, 400, $GameScreen[0], $GameScreen[1], $WS_POPUP, $WS_EX_TOOLWINDOW + $WS_EX_TOPMOST)
$status = "Running"
Global $Timer = TimerInit()
Global $runTime = 0
$But0 = GUICtrlCreateLabel("BATTLESCAPE CHOP N CUT", 0, 25, 250, -1, -1, $GUI_WS_EX_PARENTDRAG)
$StatusLabel = GUICtrlCreateLabel(" Status: " & $status, 0, 50, 120, -1, -1, $GUI_WS_EX_PARENTDRAG)
$RuntimeLabel = GUICtrlCreateLabel(" Runtime: " & $runTime, 125, 50, 125, -1, -1, $GUI_WS_EX_PARENTDRAG)
...
Func GetTime()
	sleep(1000)
	Global $runTime = TimerDiff($Timer)
	;Convert miliseconds to seconds
	$runTime = Round($runTime/1000,2)
	;if over 3600 seconds convert to hours
	If $runTime > 3600  Then
		$runTime = Round($runTime/3600,0)
			$RuntimeLabel = GUICtrlCreateLabel(" Runtime: " & $runTime & " hours", 125, 50, 125, 21, -1, $GUI_WS_EX_PARENTDRAG)
	ElseIf $runTime > 60 Then
			$runTime = Round($runTime/60,0)
			$RuntimeLabel = GUICtrlCreateLabel(" Runtime: " & $runTime & " minutes", 125, 50, 125, 21, -1, $GUI_WS_EX_PARENTDRAG)
	Else
			$runTime = Round($runTime,0)
			$RuntimeLabel = GUICtrlCreateLabel(" Runtime: " & $runTime & " seconds", 125, 50, 125, 21, -1, $GUI_WS_EX_PARENTDRAG)
	EndIf
