...
@ScriptManifest(author = "0x6B", category = Category.MINIGAMES, description = "Buys gold at the Blast Furnace and makes bars", 
name = "0xBlast_Furnace_Gold", servers = { "Zenyte" }, version = "1.0", discord = "0x6b#7707")

public class BlastFurnaceGold extends Script {
	//Script Variables
	private String version = "1.0";
	private String STATUS = null;
	private String SCRIPTNAME = "0xBlastFurnaceGold";
	...
	private Integer BF_GOLD = 0; //Varbit BLAST_FURNACE_Gold
	private Integer BF_GOLD_BAR = 0; //Varbit BLAST_FURNACE_STEEL_BAR
	
	//Script Variables
	private Integer COINS_IN_INV = 0;
	private Integer GOLD_ORE_IN_INV = 0;
	private Integer COINS_TO_ADD = 25000; //How much money to add to the coffer
	
	//NPC Variables
		//Ore dude
		private Integer ORDAN_ID  = 1560;
		private Integer GOLD_NPC = 2931; //used by conveyor belt
	...
	//Item Variables
		...
		//Gold Ore
		private Integer GOLD_ORE_ID = 444;
		//Gold Bar
		private Integer GOLD_BAR_ID = 2357;
	...
	public void onExecute() {
		...
		startxp= ctx.skills.experience(Skills.SMITHING);
	...
	public void paint(Graphics Graphs) {
			...
			if(DEBUG == true) {
				...
				g.drawString("BF_GOLD: [ " + BF_GOLD + " ]", 350, 60);
				g.drawString("BF_STEEL_BAR: [ " + BF_GOLD_BAR + " ]", 350, 80);
				g.drawString("COINS_IN_INV: [ " + COINS_IN_INV + " ]", 350, 90);
				g.drawString("GOLD_ORE_IN_INV: [ " + GOLD_ORE_IN_INV + " ]", 350, 100);
				}
	}
	...
	public void onProcess() {
		...
		BF_GOLD = ctx.getClient().getVar(Varbits.BLAST_FURNACE_GOLD_ORE);
		BF_GOLD_BAR = ctx.getClient().getVar(Varbits.BLAST_FURNACE_GOLD_BAR);
		COINS_IN_INV = ctx.inventory.populate().filter(COINS_ID).population(true); // count the number of coins in the inventory
		GOLD_ORE_IN_INV = ctx.inventory.populate().filter(GOLD_ORE_ID).population(true);
		//check if inventory is full
		if(ctx.inventory.populate().filter(GOLD_BAR_ID).population()>0) {
			Bank();
		//check if player has money
			...
			if(COINS_IN_INV > 0) { //player has money
				...
				if(COFFER_GOLD > 0) { //check if money is in coffer
					...
					if(ctx.npcs.populate().filter(GOLD_NPC).population() >0 || BF_BAR_DISPENSER == 2) { //look for GOLD on belt
						...
						//check if there is enough coal in the furnace
							if(GOLD_ORE_IN_INV >0) {
									STATUS = "Adding Gold to belt";
									//add iron to belt
									Click_Belt();
								}else {//else - no gold in inventory
									STATUS = "Buying Gold";
									//TODO buy iron
									Buy_Item(GOLD_ORE_ID);
								}//end check if gold in inventory
							}//end check if gold in BF
